...
class ShareHarfangRunner(Runner):
    ...
    def load(self):
        ...
        # [Self-play] allocate memory for opponent policy/data in training
        if self.use_selfplay:
    ...
    def run(self):
        ...
        for episode in range(episodes):
            for step in range(self.buffer_size):
                # Sample actions
                values, actions, attention_fused_weight, action_log_probs, rnn_states_actor, rnn_states_critic = self.collect(step)

                # Observation reward and next obs
                obs, share_obs, ally_attention_n, enemy_attention_n, rewards, dones, infos = self.envs.step(actions, attention_fused_weight)

                data = obs, share_obs, ally_attention_n, enemy_attention_n, actions, rewards, dones, action_log_probs, \
                    values, rnn_states_actor, rnn_states_critic
            ...
            if episode % self.log_interval == 0:
                ...
                logging.info(
                    "\n Scenario {} Algo {} Exp {} updates {}/{} episodes, total num timestamps {}/{}, FPS {}.\n"
    ...
    def warmup(self):
        # reset env
        obs, share_obs, ally_attention_n, enemy_attention_n = self.envs.reset()

        # [Self-play] divide ego/opponent of initial obs
        if self.use_selfplay:
        ...
        self.buffer.ally_attention_n[0] = ally_attention_n.copy()
        self.buffer.enemy_attention_n[0] = enemy_attention_n.copy()

    @torch.no_grad()
    def collect(self, step):
        self.policy.prep_rollout()
        values, actions, attention_fused_weight, action_log_probs, rnn_states_actor, rnn_states_critic \
            = self.policy.get_actions(np.concatenate(self.buffer.share_obs[step]),
                                      np.concatenate(self.buffer.obs[step]),
                                      np.concatenate(self.buffer.ally_attention_n[step]),
                                      np.concatenate(self.buffer.enemy_attention_n[step]),
        ...
        attention_fused_weight = np.array(np.split(_t2n(attention_fused_weight), self.n_rollout_threads))
        action_log_probs = np.array(np.split(_t2n(action_log_probs), self.n_rollout_threads))
        rnn_states_actor = np.array(np.split(_t2n(rnn_states_actor), self.n_rollout_threads))
        rnn_states_critic = np.array(np.split(_t2n(rnn_states_critic), self.n_rollout_threads))

        # [Self-play] get actions of opponent policy
        if self.use_selfplay:
    ...
    def compute(self):
        ...
        next_values = self.policy.get_values(np.concatenate(self.buffer.share_obs[-1]),
                                             np.concatenate(self.buffer.ally_attention_n[-1]),
                                             np.concatenate(self.buffer.enemy_attention_n[-1]),
    ...
    def insert(self, data: List[np.ndarray]):
        obs, share_obs, ally_attention_n, enemy_attention_n, actions, rewards, dones, action_log_probs, values, rnn_states_actor, rnn_states_critic = data
        ...
        # [Self-play] divide ego/opponent of collecting data TODO: shared_obs
        if self.use_selfplay:
            self.opponent_obs = obs[:, self.num_agents // 2:, ...]
        ...
        self.buffer.insert(obs, share_obs, ally_attention_n, enemy_attention_n, actions, rewards, masks, action_log_probs, values,
    ...
    def eval(self, total_num_steps):
        ...
        eval_obs, eval_share_obs, ally_attention_n, enemy_attention_n = self.eval_envs.reset()
    ...
    def render(self):
        ...
        for i in range(100):
            ...
            render_obs, render_share_obs, ally_attention_n, enemy_attention_n = self.envs.reset()
            ...
            if self.use_selfplay:
                ...
                self.eval_opponent_policy.actor.load_state_dict(
                    torch.load(str(self.model_dir) + f'/actor_{policy_idx}.pt'))
                self.eval_opponent_policy.prep_rollout()
                # reset obs/rnn/mask
                render_obs, render_share_obs, ally_attention_n, enemy_attention_n = self.envs.reset()
            ...
            while True:
                self.policy.prep_rollout()
                render_actions, attention_fused_weight, render_rnn_states = self.policy.act(np.concatenate(render_obs),
                                                                                            np.concatenate(ally_attention_n),
                                                                                            np.concatenate(enemy_attention_n),
                                                                                            np.concatenate(render_rnn_states),
                                                                                            np.concatenate(render_masks))
                render_actions = np.expand_dims(_t2n(render_actions), axis=0)
                attention_fused_weight = np.expand_dims(_t2n(attention_fused_weight), axis=0)
                render_rnn_states = np.expand_dims(_t2n(render_rnn_states), axis=0)

                # [Self-play] get actions of opponent policy
                if self.use_selfplay:
                    render_opponent_actions, render_opponent_rnn_states \
                        = self.eval_opponent_policy.act(np.concatenate(render_obs),
                                                        np.concatenate(ally_attention_n),
                                                        np.concatenate(enemy_attention_n),
                                                        np.concatenate(render_rnn_states),
                                                        np.concatenate(render_masks),
                ...
                # Observation reward and next obs
                render_obs, render_share_obs, ally_attention_n, enemy_attention_n, render_rewards, render_dones, render_infos = self.envs.step(
                    render_actions, attention_fused_weight)
    ...
    def save(self, episode):
        ...
        # 保存注意力模型
        self_attention_state_dict = self.policy.self_attention.state_dict()
        torch.save(self_attention_state_dict, str(self.save_dir) + '/self_attention.pt')
        cross_attention_state_dict = self.policy.cross_attention.state_dict()
        torch.save(cross_attention_state_dict, str(self.save_dir) + '/cross_attention.pt')

        # [Self-play] save policy & performance
        if self.use_selfplay:
    ...
    def load_dict(self):
        policy_actor_state_dict = torch.load(str(self.model_dir) + '/actor_latest.pt', map_location='cuda:0')
        self.policy.actor.load_state_dict(policy_actor_state_dict)
        policy_critic_state_dict = torch.load(str(self.model_dir) + '/critic_latest.pt', map_location='cuda:0')
        self.policy.critic.load_state_dict(policy_critic_state_dict)

        self_attention_state_dict = torch.load(str(self.model_dir) + '/self_attention.pt', map_location='cuda:0')
        self.policy.self_attention.load_state_dict(self_attention_state_dict)
        cross_attention_state_dict = torch.load(str(self.model_dir) + '/cross_attention.pt', map_location='cuda:0')
        self.policy.cross_attention.load_state_dict(cross_attention_state_dict)
