...
import logging
import numpy as np
from PyQt6.QtCore import Qt, QTimer, pyqtSignal, QThread, QSize, QRectF, QPointF
from PyQt6.QtGui import QSurfaceFormat, QFont, QColor, QPainter, QIcon, QBrush, QPen, QLinearGradient, QPolygonF
...
class OpenGLWidget(QOpenGLWidget):
    def __init__(self, parent=None):
        ...
        self.enemy_team_color = QColor(60, 150, 255)  # 蓝方
        self.ally_team_color = QColor(255, 40, 40)  # 红方

        # 新增：控制是否显示速度标签
        self.show_speed_labels = True

        # Day-mode tactical renderer. QSS only controls Qt widgets; the 3D viewport
        # must be styled directly inside OpenGL painting functions.
        self.day_mode = True
        self.show_viewport_hud = True
    ...
    def initializeGL(self):
        try:
            # Day-mode sky color. This is the real 3D viewport background;
            # QWidget/QSS background does not affect QOpenGLWidget rendering.
            glClearColor(0.72, 0.88, 0.98, 1.0)
            glEnable(GL_BLEND)
            ...
            # Brighter daylight lighting: high ambient + directional sunlight.
            glLightfv(GL_LIGHT0, GL_POSITION, [9000, 16000, 12000, 1])
            glLightfv(GL_LIGHT0, GL_AMBIENT, [0.58, 0.66, 0.72, 1.0])
            glLightfv(GL_LIGHT0, GL_DIFFUSE, [0.95, 0.98, 1.00, 1.0])
            glLightfv(GL_LIGHT0, GL_SPECULAR, [0.90, 0.95, 1.00, 1.0])

            # Light atmospheric perspective for day mode.
            glEnable(GL_FOG)
            glFogfv(GL_FOG_COLOR, [0.76, 0.89, 0.98, 1.0])
            glFogi(GL_FOG_MODE, GL_LINEAR)
            glFogf(GL_FOG_START, 14000.0)
            glFogf(GL_FOG_END, 72000.0)

            # Enable texture mapping
            glEnable(GL_TEXTURE_2D)

            # Log that initialization is complete
            logging.info("OpenGL initialized successfully with day tactical mode.")
        except Exception as e:
    ...
    def paintGL(self):
        try:
            ...
            # Draw the light tactical floor before aircraft.
            if self.day_mode:
                self.draw_day_ground_plane()
            ...
            # Draw readable unit bases/shadows for day mode.
            if self.day_mode:
                self.draw_plane_ground_markers()
            ...
            if self.day_mode and self.show_viewport_hud:
                self.draw_day_viewport_overlay(painter)

            painter.setPen(QColor("#0B376D"))  # Day-mode tactical label text
                ...
                if plane_instance.is_enemy:  # Only process enemy planes
                    ...
                    if screen_pos:
                        ...
                        if self.show_speed_labels:
                            ...
                            speed_color = QColor("#0EA5E9") if self.day_mode else QColor("#FBBF24")
                            painter.setPen(speed_color)
                            if self.day_mode:
                                self.draw_speed_badge(painter, int(x), int(y) + 15, speed_text)
                            else:
    ...
    def draw_day_sky_background(self):
        """
        Draw a light blue vertical gradient as a sky background.
        This is rendered in screen space before the 3D camera is applied.
        """
        try:
            glMatrixMode(GL_PROJECTION)
            glPushMatrix()
            ...
            glBegin(GL_QUADS)
            # Top: saturated clear sky
            glColor3f(0.50, 0.78, 0.96)
            glVertex2f(0, 0)
            glVertex2f(w, 0)
            # Bottom: bright horizon haze
            ...
            logging.error(f"Exception during day sky drawing: {e}")

    def draw_day_ground_plane(self):
        """
        Draw a soft translucent tactical floor below the grid.
        """
        try:
            glDisable(GL_LIGHTING)
            glDisable(GL_TEXTURE_2D)
            ...
            s = self.grid_size
            glBegin(GL_QUADS)
            glColor4f(0.86, 0.94, 1.00, 0.42)
            glVertex3f(-s, -1, -s)
            glVertex3f(s, -1, -s)
            glColor4f(0.72, 0.86, 0.96, 0.34)
            glVertex3f(s, -1, s)
            glVertex3f(-s, -1, s)
            glEnd()
            ...
            logging.error(f"Exception during day ground drawing: {e}")

    def draw_plane_ground_markers(self):
        """
        Draw simple circular bases and soft shadows under units.
        This makes aircraft readable in a bright daytime viewport.
        """
        try:
            glDisable(GL_LIGHTING)
            glDisable(GL_TEXTURE_2D)
            ...
            for plane in self.planes_dict.values():
                x, _, z = plane.position
                radius = 390.0 if plane.is_enemy else 430.0
                if plane.is_enemy:
                    color = self.enemy_team_color
                else:
                    color = self.ally_team_color

                # Soft elliptical shadow.
                glColor4f(0.06, 0.18, 0.32, 0.12)
                glBegin(GL_TRIANGLE_FAN)
                glVertex3f(x, 2, z)
                for k in range(49):
                    a = 2.0 * math.pi * k / 48.0
                    glVertex3f(x + math.cos(a) * radius * 1.08, 2, z + math.sin(a) * radius * 0.72)
                glEnd()

                # Tactical selection ring.
                glLineWidth(1.8)
                glColor4f(color.redF(), color.greenF(), color.blueF(), 0.80)
                glBegin(GL_LINE_LOOP)
                for k in range(64):
                    a = 2.0 * math.pi * k / 64.0
                    glVertex3f(x + math.cos(a) * radius, 6, z + math.sin(a) * radius)
                glEnd()

            glEnable(GL_TEXTURE_2D)
            glEnable(GL_LIGHTING)
        except Exception as e:
            logging.error(f"Exception during unit marker drawing: {e}")

    def draw_day_viewport_overlay(self, painter):
        """
        Draw daytime HUD elements that are easier to read over a bright 3D scene.
        """
        try:
            w, h = self.width(), self.height()
            painter.save()

            # Subtle sun glow in the upper right.
            painter.setPen(Qt.PenStyle.NoPen)
            painter.setBrush(QColor(255, 255, 255, 58))
            painter.drawEllipse(w - 170, 34, 94, 94)
            painter.setBrush(QColor(255, 255, 255, 26))
            painter.drawEllipse(w - 210, -6, 178, 178)

            # Soft horizon haze so the far grid feels atmospheric.
            haze = QLinearGradient(0, 0, 0, h * 0.45)
            haze.setColorAt(0.0, QColor(255, 255, 255, 72))
            haze.setColorAt(1.0, QColor(255, 255, 255, 0))
            painter.fillRect(QRectF(0, 0, w, h * 0.45), haze)

            # Compass mini-widget.
            compass_x, compass_y, compass_r = 44, 52, 26
            painter.setPen(QPen(QColor(22, 78, 130, 160), 1))
            painter.setBrush(QColor(255, 255, 255, 90))
            painter.drawEllipse(compass_x - compass_r, compass_y - compass_r, compass_r * 2, compass_r * 2)
            painter.setPen(QColor("#0F3765"))
            painter.setFont(QFont("Segoe UI", 9, QFont.Weight.Bold))
            painter.drawText(compass_x - 5, compass_y - compass_r - 6, "N")
            painter.setPen(Qt.PenStyle.NoPen)
            painter.setBrush(QColor("#0EA5E9"))
            painter.drawPolygon(QPolygonF([
                QPointF(compass_x, compass_y - 18),
                QPointF(compass_x - 11, compass_y + 11),
                QPointF(compass_x + 11, compass_y + 11),
            ]))

            # Bottom glass HUD strip.
            panel = QRectF(14, h - 88, w - 28, 74)
            painter.setBrush(QColor(20, 88, 142, 82))
            painter.setPen(QPen(QColor(180, 230, 255, 185), 1.0))
            painter.drawRoundedRect(panel, 14, 14)

            sections = [
                ("COORDINATES", "X: 1240.50\nY:  850.30\nZ:  620.10"),
                ("CAMERA", f"PITCH: {self.rotation_x: .1f}°\nYAW: {self.rotation_y: .1f}°\nDIST: {self.camera_distance: .0f}"),
                ("TIME", time.strftime("%H:%M:%S\n%Y-%m-%d", time.localtime())),
                ("SCALE", "1.0x"),
                ("VIEW MODE", "Tactical 3D"),
            ]
            col_w = panel.width() / len(sections)
            for i, (title, value) in enumerate(sections):
                x0 = panel.left() + i * col_w + 18
                if i > 0:
                    painter.setPen(QPen(QColor(130, 190, 230, 90), 1))
                    painter.drawLine(int(panel.left() + i * col_w), int(panel.top() + 12), int(panel.left() + i * col_w), int(panel.bottom() - 12))
                painter.setPen(QColor("#EAF7FF"))
                painter.setFont(QFont("Segoe UI", 8, QFont.Weight.Bold))
                painter.drawText(int(x0), int(panel.top() + 22), title)
                painter.setPen(QColor("#FFFFFF"))
                painter.setFont(QFont("Consolas", 9))
                painter.drawText(int(x0), int(panel.top() + 42), value)

            painter.restore()
        except Exception as e:
            logging.error(f"Exception during day viewport overlay drawing: {e}")

    def draw_speed_badge(self, painter, x, y, text):
        """
        Draw a compact blue speed callout for daytime readability.
        """
        try:
            painter.save()
            painter.setFont(QFont("Segoe UI", 8, QFont.Weight.Bold))
            fm = painter.fontMetrics()
            width = max(88, fm.horizontalAdvance(text) + 20)
            rect = QRectF(x - width / 2, y - 12, width, 24)
            painter.setBrush(QColor(14, 165, 233, 168))
            painter.setPen(QPen(QColor(226, 246, 255, 210), 1))
            painter.drawRoundedRect(rect, 7, 7)
            painter.setPen(QColor("#FFFFFF"))
            painter.drawText(rect, Qt.AlignmentFlag.AlignCenter, text)
            painter.restore()
        except Exception as e:
            logging.error(f"Exception during speed badge drawing: {e}")

    def draw_grid(self):
        """
        Draw a light daytime tactical grid located at y=0.
        Major lines are slightly stronger than minor lines.
        """
        try:
            glDisable(GL_LIGHTING)
            glDisable(GL_TEXTURE_2D)
            ...
            major_every = self.grid_spacing * 5

            # Draw lines parallel to Z-axis
            for x in range(-self.grid_size, self.grid_size + self.grid_spacing, self.grid_spacing):
                is_major = (x % major_every == 0)
                glLineWidth(1.15 if is_major else 0.65)
                if self.day_mode:
                    glColor4f(0.30, 0.62, 0.86, 0.48 if is_major else 0.25)
                else:
                    glColor4f(0.20, 0.34, 0.52, 0.35)
                glBegin(GL_LINES)
                glVertex3f(x, 0, -self.grid_size)
                glVertex3f(x, 0, self.grid_size)
                glEnd()

            # Draw lines parallel to X-axis
            for z in range(-self.grid_size, self.grid_size + self.grid_spacing, self.grid_spacing):
                is_major = (z % major_every == 0)
                glLineWidth(1.15 if is_major else 0.65)
                if self.day_mode:
                    glColor4f(0.30, 0.62, 0.86, 0.48 if is_major else 0.25)
                else:
