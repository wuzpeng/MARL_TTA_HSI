...
class PPOTrainer():
    ...
    def ppo_update(self, policy: PPOPolicy, sample):
        ...
        obs_batch, share_obs_batch, ally_attention_batch, enemy_attention_batch, actions_batch, masks_batch,\
            active_masks_batch, old_action_log_probs_batch, advantages_batch, \
        ...
        for param_group in policy.optimizer.param_groups:
            if param_group['lr'] < policy.lr_min:
                param_group['lr'] = policy.lr_min
            else:
                policy.scheduler.step()  # 更新学习率
    ...
    def train(self, policy: PPOPolicy, buffer: SharedReplayBuffer):
        ...
        for _ in range(self.ppo_epoch):
            ...
            for sample in data_generator:
                ...
                train_info['lr'] = policy.optimizer.param_groups[0]['lr']  # 学习率
