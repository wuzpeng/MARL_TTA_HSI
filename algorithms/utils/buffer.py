...
class SharedReplayBuffer(ReplayBuffer):
    ...
    def __init__(self, args, num_agents, obs_space, share_obs_space, act_space):
        ...
        self.ally_attention_n = np.zeros((self.buffer_size + 1, self.n_rollout_threads, self.num_agents, 7), dtype=np.float32)
        self.enemy_attention_n = np.zeros((self.buffer_size + 1, self.n_rollout_threads, self.num_agents, 7), dtype=np.float32)
    ...
    def insert(self,
               ...
               ally_attention_n: np.ndarray,
               enemy_attention_n: np.ndarray,
               actions: np.ndarray,
               rewards: np.ndarray,
               masks: np.ndarray,
               action_log_probs: np.ndarray,
        ...
        Args:
            obs:                o_{t+1}
            share_obs:          s_{t+1}
            ally_attention_n    己方注意力特征
            enemy_attention_n   敌方注意力特征
            actions:            a_{t}
            rewards:            r_{t}
            masks:              1 - done_{t}
            action_log_probs:   log_prob(a_{t})
        ...
        self.ally_attention_n[self.step + 1] = ally_attention_n.copy()
        self.enemy_attention_n[self.step + 1] = enemy_attention_n.copy()
    ...
    def recurrent_generator(self, advantages: np.ndarray, num_mini_batch: int, data_chunk_length: int):
        ...
        ally_attention_n = self._cast(self.ally_attention_n[:-1])
        enemy_attention_n = self._cast(self.enemy_attention_n[:-1])
        ...
        for indices in sampler:
            ...
            ally_attention_batch = []
            enemy_attention_batch = []
            ...
            for index in indices:
                ...
                ally_attention_batch.append(ally_attention_n[ind:ind + data_chunk_length])
                enemy_attention_batch.append(enemy_attention_n[ind:ind + data_chunk_length])
            ...
            ally_attention_batch = np.stack(ally_attention_batch, axis=1)
            enemy_attention_batch = np.stack(enemy_attention_batch, axis=1)
            ...
            ally_attention_batch = self._flatten(L, N, ally_attention_batch)
            enemy_attention_batch = self._flatten(L, N, enemy_attention_batch)
            ...
            yield obs_batch, share_obs_batch, ally_attention_batch, enemy_attention_batch, actions_batch,\
                masks_batch, active_masks_batch, old_action_log_probs_batch, advantages_batch, returns_batch,\
                value_preds_batch, rnn_states_actor_batch, rnn_states_critic_batch
